...
fn main() -> Result<()> {
    ...
    for line in input_line {
        match line {
            Ok(levels_string) => {
                let report = Report::new(levels_string)?;
                if Report::is_safe(&report.levels) {
                    safe_count += 1;
                    dampener_safe_count += 1
                } else if Report::is_safe_with_dampener(&report.levels) {
                    dampener_safe_count += 1;
                }
            }
            Err(e) => {
...
impl Report {
    fn new(levels_string: String) -> Result<Report> {
    ...
    fn is_safe_with_dampener(levels: &[i64]) -> bool {
        ...
        for i in 0..levels.len() {
            ...
            dampener_safe.push(Report::is_safe(&dampened_levels));
        }
